...
<template>
    <div class="flex justify-evenly">
        <Button v-if="!cartStore.cartItems.find((item) => item.id === props.product.id)"
            class="bg-blue-500 hover:bg-blue-600 rounded-md p-2 px-4" @click="cartStore.addToCart(props.product)">
            Add to Cart
        </Button>
        <div v-else class="flex justify-evenly items-center">
            ...
            <span class="w-[30px] text-black text-center">{{ cartItems.find((item) => item.id ===
                props.product.id)?.quantity }}</span>
            <span
